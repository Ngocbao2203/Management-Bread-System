...
const FeaturedCombo = () => {
  ...
  const scrollToProduct = (index) => {
    if (scrollRef.current) {
      ...
      const normalizedIndex = index % combos.length
      if (normalizedIndex < 0) {
        setCurrentIndex(combos.length - 1) // Quay về sản phẩm cuối nếu index âm
        const scrollAmount = (cardWidth + gap) * (combos.length - 1)
  ...
  const snapToNearestProduct = (scrollPosition) => {
    ...
    nearestProduct =
      ((nearestProduct % combos.length) + combos.length) % combos.length
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      ...
      setCurrentIndex(
        ((newIndex % combos.length) + combos.length) % combos.length
      )
    ...
    const handleMouseMove = (e) => {
      ...
      animationFrameId = requestAnimationFrame(() => {
        ...
        const maxScroll = (combos.length - 1) * (cardWidth + gap)
